...
func (opts serverCreateOpts) ToServerCreateMap() (map[string]interface{}, error) {
	...
	if len(opts.Networks) > 0 {
		...
		for i, nets := range opts.Networks {
			networks[i] = make(map[string]interface{})
			if nets.UUID != "" {
				networks[i]["uuid"] = nets.UUID
			}
			if nets.Port != "" {
				networks[i]["port"] = nets.Port
			}
			if nets.FixedIP != "" {
				networks[i]["fixed_ip"] = nets.FixedIP
			}
		}
		b["networks"] = networks
	}
...
func (client *Client) ResizeHost(id string, templateID string) (*model.Host, error) {
	return client.osclt.ResizeHost(id, templateID)
}
...
func validatehostName(req model.HostRequest) (bool, error) {
	...
	e := s.Validate(req)
	if e.HasErrors() {
		resourceErrors, _ := e.GetErrorsByKey("ResourceName")
		var errs []string
...
func (client *Client) complementHost(host *model.Host, server *servers.Server) error {
	...
	for netid, netname := range hostNetworkV1.NetworksByID {
		if netname == "" {
			nets, err := client.GetNetwork(netid)
			if err != nil {
				log.Errorf("failed to get network '%s'", netid)
				continue
			}
			hostNetworkV1.NetworksByID[netid] = nets.Name
			hostNetworkV1.NetworksByName[nets.Name] = netid
		}
	}
